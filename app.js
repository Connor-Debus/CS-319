...
const defaultState = {
  ...
  assignments: [
    { id: 'a1', title: 'Binary search analysis', courseId: 'c1', due: '2026-09-07', priority: 'high', status: 'todo', grade: null },
    { id: 'a2', title: 'Reading response: public space', courseId: 'c2', due: '2026-09-09', priority: 'medium', status: 'todo', grade: null },
    { id: 'a3', title: 'Problem set 04', courseId: 'c3', due: '2026-09-11', priority: 'low', status: 'todo', grade: null },
    { id: 'a4', title: 'Interface critique', courseId: 'c2', due: '2026-09-03', priority: 'medium', status: 'completed', grade: null }
  ]
};
...
function loadState() {
  try { const saved = JSON.parse(localStorage.getItem(STORAGE_KEY)) || structuredClone(defaultState); saved.assignments = saved.assignments.map((item) => ({ ...item, grade: item.grade ?? null })); return saved; }
...
function renderStats() {
  ...
  const overdue = incomplete.filter((item) => item.due < today()).length;
  const graded = assignments.filter((item) => typeof item.grade === 'number' && Number.isFinite(item.grade));
  const average = graded.length ? Math.round(graded.reduce((sum, item) => sum + item.grade, 0) / graded.length) : null;
  $('#total-value').textContent = assignments.length;
  $('#completed-value').textContent = complete;
  $('#completed-count').textContent = `${complete} of ${assignments.length}`;
  $('#remaining-value').textContent = incomplete.length;
  $('#overdue-value').textContent = overdue;
  $('#average-value').textContent = average === null ? '—' : `${average}%`;
  $('#graded-count').textContent = graded.length;
}
function today() { return new Date().toISOString().slice(0, 10); }
function isOverdue(item) { return item.status !== 'completed' && item.due < today(); }
...
document.addEventListener('change', (event) => { const target = event.target; if (target.dataset.priority) updateAssignment(target.dataset.priority, { priority: target.value }); if (target.dataset.status) updateAssignment(target.dataset.status, { status: target.value }); if (target.dataset.grade) updateAssignment(target.dataset.grade, { grade: target.value === '' ? null : Number(target.value) }); });
